...
{
	struct sock *sk = sock->sk;
	char name[sizeof(((struct sockaddr_legacy *)uaddr)->sa_data) + 1];

	/*
	 *	Check legality
	 */

	if (addr_len != sizeof(struct sockaddr_legacy))
		return -EINVAL;
	/* uaddr->sa_data comes from the userspace, it's not guaranteed to be
	 * zero-terminated.
	 */
	memcpy(name, uaddr->sa_data, sizeof(((struct sockaddr_legacy *)uaddr)->sa_data));
	name[sizeof(((struct sockaddr_legacy *)uaddr)->sa_data)] = 0;
			       ...
			       int peer)
{
	...
	memset(uaddr->sa_data, 0, sizeof(((struct sockaddr_legacy *)uaddr)->sa_data));
	rcu_read_lock();
	dev = dev_get_by_index_rcu(sock_net(sk), READ_ONCE(pkt_sk(sk)->ifindex));
	if (dev)
		strscpy(uaddr->sa_data, dev->name, sizeof(((struct sockaddr_legacy *)uaddr)->sa_data));
	rcu_read_unlock();

	return sizeof(*(struct sockaddr_legacy *)uaddr);
}
