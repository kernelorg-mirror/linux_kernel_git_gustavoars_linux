...
static void usb_ep1_command_reply_dispatch (struct urb* urb)
{
	...
	ret = usb_submit_urb(container_of(&cdev->ep1_in_urb, struct urb, hdr),
		             GFP_ATOMIC);
	if (ret < 0)
...
static int init_card(struct snd_usb_caiaqdev *cdev)
{
	...
	usb_init_urb(container_of(&cdev->ep1_in_urb, struct urb, hdr));
	usb_init_urb(container_of(&cdev->midi_out_urb, struct urb, hdr));

	usb_fill_bulk_urb(container_of(&cdev->ep1_in_urb, struct urb, hdr),
			  usb_dev,
			  usb_rcvbulkpipe(usb_dev, 0x1),
			  cdev->ep1_in_buf, EP1_BUFSIZE,
			  usb_ep1_command_reply_dispatch, cdev);

	usb_fill_bulk_urb(container_of(&cdev->midi_out_urb, struct urb, hdr),
			  usb_dev,
	...
	if (usb_urb_ep_type_check(container_of(&cdev->ep1_in_urb, struct urb, hdr)) ||
	    usb_urb_ep_type_check(container_of(&cdev->midi_out_urb, struct urb, hdr))) {
	...
	if (usb_submit_urb(container_of(&cdev->ep1_in_urb, struct urb, hdr), GFP_KERNEL) != 0)
		return -EIO;
 ...
 err_kill_urb:
	usb_kill_urb(container_of(&cdev->ep1_in_urb, struct urb, hdr));
	return err;
}
...
static void snd_disconnect(struct usb_interface *intf)
{
	...
	usb_kill_urb(container_of(&cdev->ep1_in_urb, struct urb, hdr));
	usb_kill_urb(container_of(&cdev->midi_out_urb, struct urb, hdr));
