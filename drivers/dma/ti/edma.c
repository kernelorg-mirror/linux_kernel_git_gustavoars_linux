	...
	unsigned long tx_flags, void *context)
{
	...
	edesc = kzalloc(flex_struct_size(edesc, pset, sg_len), GFP_ATOMIC);
	if (!edesc)
		return NULL;
	...
	size_t len, unsigned long tx_flags)
{
	...
	edesc = kzalloc(flex_struct_size(edesc, pset, nslots), GFP_ATOMIC);
	if (!edesc)
		return NULL;
			  ...
			  unsigned long tx_flags)
{
	...
	edesc = kzalloc(flex_struct_size(edesc, pset, 1), GFP_ATOMIC);
	if (!edesc)
		return NULL;
