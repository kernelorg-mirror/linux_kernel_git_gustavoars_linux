...
static void __init imx7ulp_clk_scg1_init(struct device_node *np)
{
	...
	clk_data = kzalloc(flex_struct_size(clk_data, hws, IMX7ULP_CLK_SCG1_END),
			   GFP_KERNEL);
	if (!clk_data)
		return;
...
static void __init imx7ulp_clk_pcc2_init(struct device_node *np)
{
	...
	clk_data = kzalloc(flex_struct_size(clk_data, hws, IMX7ULP_CLK_PCC2_END),
			   GFP_KERNEL);
	if (!clk_data)
		return;
...
static void __init imx7ulp_clk_pcc3_init(struct device_node *np)
{
	...
	clk_data = kzalloc(flex_struct_size(clk_data, hws, IMX7ULP_CLK_PCC3_END),
			   GFP_KERNEL);
	if (!clk_data)
		return;
...
static void __init imx7ulp_clk_smc1_init(struct device_node *np)
{
	...
	clk_data = kzalloc(flex_struct_size(clk_data, hws, IMX7ULP_CLK_SMC1_END),
			   GFP_KERNEL);
	if (!clk_data)
		return;
