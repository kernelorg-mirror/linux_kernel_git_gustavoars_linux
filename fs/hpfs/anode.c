...
secno hpfs_add_sector_to_btree(struct super_block *s, secno node, int fnod, unsigned fsecno)
{
	...
	while (up != (anode_secno)-1) {
		...
		if ((new_anode = hpfs_alloc_anode(s, a, &na, &bh))) {
			...
			GET_BTREE(&anode->btree)->u.internal[0].down = cpu_to_le32(a);
			GET_BTREE(&anode->btree)->u.internal[0].file_secno = cpu_to_le32(-1);
			mark_buffer_dirty(bh);
			brelse(bh);
	...
	GET_BTREE(&ranode->btree)->n_free_nodes = (bp_internal(GET_BTREE(&ranode->btree)) ? 60 : 40) - GET_BTREE(&ranode->btree)->n_used_nodes;
	if (bp_internal(GET_BTREE(&ranode->btree))) for (n = 0; n < GET_BTREE(&ranode->btree)->n_used_nodes; n++) {
		struct anode *unode;
...
static secno anode_lookup(struct super_block *s, anode_secno a, unsigned sec)
{
	...
	struct buffer_head *bh;
	if (!(anode = hpfs_map_anode(s, a, &bh))) return -1;
	return hpfs_bplus_lookup(s, NULL, GET_BTREE(&anode->btree), sec, bh);
}
...
void hpfs_ea_remove(struct super_block *s, secno a, int ano, unsigned len)
{
	...
	if (ano) {
		...
		hpfs_remove_btree(s, GET_BTREE(&anode->btree));
		brelse(bh);
...
void hpfs_remove_fnode(struct super_block *s, fnode_secno fno)
{
	...
	if (!fnode_is_dir(fnode)) hpfs_remove_btree(s, GET_BTREE(&fnode->btree));
